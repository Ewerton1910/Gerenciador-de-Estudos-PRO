const CLOUD_NAME = "dgagemtqg";
      ...
      db.ref("studyData").on("value", (snap) => {
        ...
        if (data) {
          ...
          // Atualiza os campos do painel com o que veio do banco
          document.getElementById("alarmInterval").value =
            studyData.settings.alarmInterval;
          document.getElementById("alarmActive").checked =
            studyData.settings.alarmActive;
        }
        render();
      });

      function updateAlarmSettings() {
        studyData.settings.alarmInterval = parseInt(
          document.getElementById("alarmInterval").value
        );
        studyData.settings.alarmActive =
          document.getElementById("alarmActive").checked;
      ...
      async function handleUpload() {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          try {
            ...
            if (data.secure_url) {
              let folder = studyData.folders.find(
              ...
              document.getElementById(
                "up-" + i
              ).innerHTML = `<span>✅ Pronto</span>`;
            }
          } catch (e) {
            document.getElementById(
              "up-" + i
            ).innerHTML = `<span>❌ Erro</span>`;
          }
        }
      ...
      function render() {
        ...
        dashboard.innerHTML = studyData.folders
          .map((f) => {
            ...
            // Lógica Personalizada de Revisão
            const needsRevision =
              config.alarmActive &&
              f.files.some(
                (file) =>
          ...
          grid.innerHTML =
            ...
            folder.files
              .map((file) => {
                ...
                return `
                <div class="card" onclick="openPDF('${folder.id}', '${
                  file.id
                }')">
          ...
          grid.innerHTML = filtered
            .map((f) => {
              ...
              const badges = (f.days || [])
                ...
                .join("");
              return `<div class="card" onclick="activeFolderId='${
                f.id
              }'; render()">
                    ${
                      needsRevision
